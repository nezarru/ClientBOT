...
async def reply(client, chat_id, text):
    for attempt in range(3):
        try:
            ...
            print(f"[reply] попытка {attempt+1} ошибка: {e}")
...
def register_handlers(client):
    ...
    async def cmd_menu(event):
        ...
        print(f"[{me.username}] /start от {uid} в чате {event.chat_id}")
    ...
    async def cmd_on(event):
        if not event.is_private:
            return
        me = await client.get_me()
        print(f"[{me.username}] /on от {event.sender_id}")
    ...
    async def cmd_off(event):
        ...
        uid = event.sender_id
        me = await client.get_me()
        print(f"[{me.username}] /off от {uid}")
    ...
    async def cmd_send(event):
        ...
        uid = event.sender_id
        me = await client.get_me()
        print(f"[{me.username}] /send от {uid}")
        if not is_bot_enabled(uid):
            await reply(client, event.chat_id, "🔴 Бот выключен. /on — включить")
            return
        if uid in active_tasks:
    ...
    async def cmd_stop(event):
        ...
        uid = event.sender_id
        me = await client.get_me()
        print(f"[{me.username}] /stop от {uid}")
        if uid in active_tasks:
            active_tasks[uid].cancel()
...
async def run_client(acc):
    while True:
        try:
            ...
            me = await c.get_me()
            print(f"✅ Аккаунт #{acc['index']}: {me.first_name} (@{me.username}) подключён")
            await c.run_until_disconnected()
            print(f"⚠️ Аккаунт #{acc['index']} отключился, переподключаюсь...")
        except Exception as e:
            print(f"❌ Аккаунт #{acc['index']} ошибка: {e}")
        await asyncio.sleep(5)

async def main():
    accounts = load_accounts()
    print(f"\n🚀 Запускаю {len(accounts)} аккаунт(ов).")
    print("Напиши /start или /menu в любой приватный чат чтобы открыть меню.\n")
